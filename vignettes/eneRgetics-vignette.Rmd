---
title: "Getting started with eneRgetics"
author: "Camilo Carneiro"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteEngine{rmarkdown::render}
  %\VignetteIndexEntry{Getting started with eneRgetics}
...
Imagine we want to estimate the maintenance metabolism of the Whimbrel (*Numenius phaeopus*) for a period of time during the non-breeding season of 2016-2017 in the Banc d'Arguin, Mauritania (latitude N 19.9,longitude W 16.3). Let's assume that period to be from 7 to 14 January 2017.
...
### 1. Field data
First we need to know how much time whimbrels spend on each microhabitat. Microhabitat use is related to their activity (*e.g.* foraging during low tide in open tidal flats or roosting during high tide). To do so, we have recorded the percentage of birds on each activity, at given time intervals - in this case the proportion of birds roosting and not roosting (*i.e.* foraging, preening, interating) - over the course of the non-breeding season. We end up with the following type of data:
...
time_budget$date <- as.POSIXct(paste(time_budget$date, time_budget$time), format = "%Y-%m-%d %H:%M", tz = "GMT")
...
tides_jan <- get_tides(lat = latitude, lon = longitude, start_date = start, end_date = end, tzone = time_zone, API_key = key)
```
...
weather_jan <- get_weather(lat = latitude, lon = longitude, start_date = start_date, end_date = end_date, tzone = time_zone, reli_score = reli_score, API_key = API_key, cams_user = cams_user)
```
...
Maintenance_metabolism <- mmaint(weatherData = weatherData, tide_table = tide_table, timeBudget = time_budget, breast_height = breast_height, body_mass = body_mass)
```
...
Maintenance_no_tides <- mmaint(weatherData = weatherData, timeBudget = time_budget, breast_height = breast_height, body_mass = body_mass)
```
